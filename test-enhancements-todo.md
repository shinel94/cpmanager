...
- `enhance-playable.md` (오디오 재생 및 MIDI 내보내기 명세서)
- `enhance-playable-todo.md` (오디오/MIDI 작업 목록 및 게이트 점검표)
- `wave_log/` (기본 Wave 0~6, FE Enhance 0~4, BE Enhance 0~5, Playable Wave 0~7 아카이브)
...
1. **제로 리그레션 (Zero Regression Policy)**:
   - 신규 테스트를 작성하거나 인프라를 확장할 때, 현재 100% 통과 중인 **기존 101개 테스트는 단 하나도 실패하지 않아야 한다**.
...
3. **사용자 행동 중심 검증 (Behavior over Implementation)**:
   - 컴포넌트 내부의 private state 변수명을 직접 단언(Assert)하지 않고, 실제 화면에 나타나는 텍스트(`진행: C - Am...`), 배지(`세컨더리 도미넌트`, `[▶ 2박]`), 버튼 활성화(`disabled`), 대화상자 알림, 오디오 발음 피드백을 기준으로 테스트하여 리팩터링에 견고한 테스트를 유지한다.
4. **회귀 이슈 선제 방어 (Empirical Regression Defense)**:
   - 개발 과정(Wave Logs)에서 실제로 발생했던 14대 이슈(무한 렌더링 루프, 텐션 비호환, 전체 와일드카드, beat 누락, 음역대 점프, SSR window 충돌, 재생 중 코드 수정 지연, MIDI 빈 마디 붕괴 등)를 독립된 회귀 테스트 케이스로 영구 보존한다.
...
│ 🎯 게이트 [G1-TEST]: 기존 101개 + 신규 BE 테스트 100% 통과 (105개+ pass) │
└────────────────────────────────────┬────────────────────────────────────┘
                                     │
                                     ▼
┌─────────────────────────────────────────────────────────────────────────┐
...
│ - PlaybackToolbar & BarCard: 실시간 재생/정지, BPM 슬라이더, 청음 연동  │
│ - MidiExportButton: 드롭다운 및 SMF Type 1 다운로드 트리거               │
...
│ 🎯 게이트 [G3-TEST]: Chromium 헤드리스 환경에서 6대 시나리오 100% 통과  │
└────────────────────────────────────┬────────────────────────────────────┘
                                     │
                                     ▼
┌─────────────────────────────────────────────────────────────────────────┐
...
`[G1-TEST]` 기존 101개 테스트 100% 유지 + Phase 1 신규 BE 4개 테스트 파일이 모두 통과하여 총 테스트 수가 105개 이상이어야 한다.
...
- [ ] `[P]` **Task 2.3: `InlineChordBuilder` 인터랙션 및 텐션/전위 호환성 가드 테스트**
  ...
  - **세부 검증 내용**:
    ...
    - 인라인 빌더의 `[🔊 청음]` 버튼 클릭 시 실시간 구성된 화음 음표가 청음 발음되는지 검증
...
- [ ] `[P]` **Task 2.4: `RecommendationPanel` 150ms 디바운스 & 무한 렌더링 방어 테스트**
  ...
  - **세부 검증 내용**:
    ...
    - 추천 카드의 `[🔊 청음]` 버튼 클릭 시 4마디 화음 순차 미리듣기 동작 검증
...
- [ ] `[P]` **Task 2.6: `PlaybackToolbar` & `BarCard` 재생 제어, 실시간 재생헤드 및 단일 청음 테스트**
  - **대상 파일**: `test/components/playback-toolbar.test.tsx` (신규)
  - **세부 검증 내용**:
    - `[▶ 재생]` 클릭 시 `AudioScheduler.play()` 호출 및 `[⏸ 일시정지]` 토글, `[⏹ 정지]` 클릭 시 정지 상태 리셋
    - BPM 슬라이더(40~240) 및 숫자 인풋 조작 시 연주 끊김 없이 실시간 템포 동기화
    - `[🔁 반복]` 및 `[⏱️ 메트로놈]` 토글 시 상태 반전 및 로컬스토리지 저장
    - `BarCard`에 `isPlaying={true}` 및 `playingBeat={3}` 전달 시 에메랄드 테두리(`ring-emerald-500`), `[▶ 3박]` 배지 및 4-beat dot meter 3번째 활성화 렌더링 검증
    - 마디 카드 헤더 및 4박 분할 뷰 각 박자별 `🔊` 청음 버튼 클릭 시 단일 화음 발음 핸들러 트리거 검증

- [ ] `[P]` **Task 2.7: `MidiExportButton` SMF Type 1 다운로드 및 토스트 피드백 테스트**
  - **대상 파일**: `test/components/midi-export-button.test.tsx` (신규)
  - **세부 검증 내용**:
    - `[💾 MIDI 내보내기 ▾]` 클릭 시 드롭다운 팝업 노출 (전체 곡 내보내기 + 개별 섹션 목록)
    - '전체 곡 내보내기' 클릭 시 `downloadProjectMidi` 호출 및 다운로드 시작 토스트 알림 노출
    - 특정 섹션 선택 시 `targetSectionId` 옵션과 함께 섹션 전용 MIDI 다운로드 트리거 검증

`[G2-TEST]` React 19 컴포넌트를 실제 DOM 환경에 마운트하여 단축키, 인라인 빌더, 뷰 모드 토글, 오디오 툴바/재생헤드, MIDI 내보내기, 디바운스 및 모달 인터랙션 테스트가 모두 통과해야 한다.
...
- [ ] `[P]` **Task 3.1: [E2E-01] 10분 팝 작곡 & 청음 & MIDI 내보내기 골든 패스 자동화**
  ...
  - **시나리오**:
    ...
    7. 상단 `PlaybackToolbar`의 `[▶ 재생]` 버튼 클릭 → 현재 연주 마디(`BarCard`)가 초록색 테두리로 하이라이트되며 실시간 이동하고, `[⏱️ 메트로놈]` 클릭 시 비트 클릭음 연동 확인
    8. 재생 중에 BPM 슬라이더를 120에서 136으로 이동 → 음악 중단 없이 실시간 템포 반영 확인
    9. `[💾 MIDI 내보내기 ▾]` 클릭 → '전체 곡 내보내기' 클릭 → 브라우저 파일 다운로드(`*.mid`) 이벤트 및 토스트 알림 확인
    10. 상단 `[프로젝트 저장]` 클릭 → Toast 알림 확인
    11. `page.reload()` 후 저장했던 G Major 조성, 템포(136 BPM), 8마디 코드가 100% 동일하게 복원됨을 검증
...
- [ ] `[P]` **Task 3.6: [E2E-06] 오디오 실시간 재생 제어 및 핫 리스케줄링 자동화**
  - **대상 파일**: `e2e/06-audio-playback.spec.ts` (신규)
  - **시나리오**:
    1. 차트에 4마디 코드 입력 후 `[▶ 재생]` 클릭
    2. 연주 도중 3번째 마디 코드를 변경 시 오디오 끊김 없이 즉시 핫 리스케줄링되어 변경된 코드로 발음됨 확인
    3. 재생 범위를 `"선택 섹션"`으로 설정하고 `[🔁 반복]` 활성화 시 섹션 루프 확인
    4. 마디 카드의 `🔊` 단일 청음 클릭 시 재생 충돌 없이 즉시 발음 검증

`[G3-TEST]` 실제 헤드리스 브라우저 환경에서 6대 E2E 사용자 여정 스크립트가 100% 통과해야 한다.
...
- [ ] **Task 4.1: 공통 테스트 팩토리 및 헬퍼 일원화**
  ...
  - 기존 30개 테스트 파일의 중복 팩토리 함수 점진적 치환
...
# 2. 현재 전체 단위/통합 테스트 실행 (101 passed)
npm test
